...
class MyApp:
    def __init__(self, root):
        ...
        image = Image.open(r"gui\Quantum Circuit Simulation.png")
        ...
        self.create_button(text='S', x=49, y=603, command=lambda: self.clickButton('S'))
        self.create_button(text='T', x=156, y=603, command=lambda: self.clickButton('T'))
        self.create_button(text='|0>', x=263, y=603, command=lambda: self.clickButton('Ketzero'))
        self.create_button(text='|1>', x=49, y=722, command=lambda: self.clickButton('Ketone'))
        ...
        self.create_button(text='BlcH', x=484, y=425, command=lambda: self.plotBlochSphere())
        self.create_button(text='BarP', x=484, y=516, command=lambda: self.plotBarGraph())  # New button for bar graph
    ...
    def initialize_log_file(self):
        ...
        filename = f"history\\{current_date}.txt"
        if not os.path.exists(filename):
            with open(filename, "w") as file:
                file.write(f"history\\{current_date}\n")
        return filename
    ...
    def equalTo(self):
        gates = {"X": X, "Y": Y, "Z": Z, "H": H,
                 ...
                 "Ketzero": Ketzero, "Ketone": Ketone,'S':S,'T':T}
        result = gates['I']()
